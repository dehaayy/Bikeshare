...
final_weath <- data.frame(date = as.character(df$dt_iso), temp = df$temp,
                          ...
                          humidity = df$humidity, windspd = df$wind_speed, weather_desc = df$weather_description )
View(final_weath)

View(data.frame(df$weather_description, zxc$weather_description))

#removes duplicated values
df <- weather_data[!duplicated(weather_data$date),]
View(df)

fwrite(df,"/Users/dehaay/Desktop/Finalized_data/fullweatherdata.csv", row.names = FALSE)
