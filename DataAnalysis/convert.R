...
for( i in sub_paths) {
  ...
  if (!is.POSIXct(data_being_read$started_at) || !is.POSIXct(data_being_read$ended_at)) {
...
fwrite(by_hour_data_start,"/Users/dehaay/Desktop/CitiBike_Outputs/deha_citibike_by_hour_data_START.csv", row.names = FALSE)
fwrite(by_hour_data_end,"/Users/dehaay/Desktop/CitiBike_Outputs/deha_citibike_by_hour_data_END.csv", row.names = FALSE)
fwrite(by_hour_netflow,"/Users/dehaay/Desktop/CitiBike_Outputs/deha_citibike_NETFLOW.csv", row.names = FALSE)


##Checking for the accuracy of the table values: 
length(which(data[which(data$hour_interval == 18 
                        & data$day == 8
                        & data$weekday == 5 
                        & data$month == 12
                        & data$year == 2017  ) ,]$end_station_name
