...
for( i in sub_paths) {
  ...
  if(colnames(data_being_read)[1] == "tripduration") {
    ...
    data_being_read <- data.frame(data_being_read$starttime,data_being_read$stoptime,data_being_read$`start station name`,
    ...
    data_being_read <- data.frame(data_being_read$`Start Time`,
                                  data_being_read$`Stop Time`, data_being_read$`Start Station Name`,
                                  data_being_read$`End Station Name`)
    colnames(data_being_read) <- c("started_at"	,
                                   "ended_at",	"start_station_name",	
                                   "end_station_name")
    ...
    data_being_read <- data.frame(data_being_read$`started_at`,
                                  data_being_read$`ended_at`, data_being_read$`start_station_name`,
...
fwrite(by_hour_data_start,"/Users/dehaay/Desktop/newdatastorage/deha_citibike_by_hour_data_START.csv", row.names = FALSE)
fwrite(by_hour_data_end,"/Users/dehaay/Desktop/newdatastorage/deha_citibike_by_hour_data_END.csv", row.names = FALSE)



View(by_hour_data_start)
View(by_hour_data_end)
View(by_hour_netflow)
